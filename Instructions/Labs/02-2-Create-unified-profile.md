...
5. 顧客プロファイルを表すテーブルを選択します。 表は次のとおりです。
   ...
   - 顧客 (ロイヤルティ)
6. **適用**を選択します。
7. テーブル内の標準モデルの種類に対するソース テーブルのマッピングを確認します。
...
3. 規則の **[名前]** に `FullName, Email` と入力します。
...
1. Customer Insights で、左側のナビゲーション メニューの **[データ]** を展開し、**[統合]** を選択します。
2. [照合ルール] で **[編集]** を選択します。
3. **Customers: Loyalty** ルールを展開し、**[編集]** ボタンを選択して、**FullName, Emaill** の条件ウィンドウを開きます。
4. [条件 1] で **[プレビュー]** を選択し、値をメモします。次に、条件 1 の精度スライダーを **[高]** から **[低]** に動かして、**[完了]** を選択します。
...
4. 結合された **FirstName** 属性を展開します。 [電子商取引連絡先] の [FirstName] 属性が 1 位にランク付けされていることがわかります。 これは、LoyaltyScheme と eCommerce の顧客プロファイルが一致している場合に、eCommerce: Contacts から取得された FirstName がプライマリとなることを示しています。
5. **[次へ]** を選択し、**[顧客プロファイルの作成]** を選択します。
6. プロセスが完了するまで待ちます。
...
2. 顧客の検索を使用して、統合された顧客プロファイルに関連するテキスト属性を検索します。 (例: 「24502」を検索すると、すべてのテキスト属性が検索され、一致と部分一致が返されます)。
